...
app.controller('MyCtrl2', function($scope, $http, $location) {
    $scope.fillTable = function() {
		...
		$http({
			method : "GET",
			url : "https://insuredefender.herokuapp.com/Defender/agent/",
		})
		.then(
	...
	app.controller('myCtrl', function($scope, $http, $location) {
		$scope.Login = function(userName, email, pass) {
			$http({
						method : "POST",
						url : "https://insuredefender.herokuapp.com/Defender/Admin/login",
		...
		$scope.createAdmin = function(userName, email, pass) {
			...
			$http({
				method : "POST",
				url : "https://insuredefender.herokuapp.com/Defender/Admin/",
		...
		$scope.error = function() {
			$http({
				method : "GET",
				url : "https://insuredefender.herokuapp.com/Defender/Upload/error",
			})
			.then(
					function mySuccess(response) {
						console.log("Admin got:"+response);
